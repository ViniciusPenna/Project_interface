...
if(isset($authChk) == true) {
  if($authUser) {
    ...
    $displayUser = $row['lpa_user_username'];
  } else {
...
function build_navBlock() { 
    ...
    <div id="navBlock">
      ...
      <div class="navItem" onclick="navMan('PI.php')">PERSONAL INFO</div>
      <div class="navItem" onclick="navMan('Mashup.php')">ABOUT</div>
	  <?PHP
      if($isAdmin) {
		?>
...
function reg_log($username, $message)
{
    $log = "LOG - IP address:".$_SERVER['REMOTE_ADDR'].'_'.date("F j,Y,g:i a") .PHP_EOL . "User: " .$username ."- Action:" .$message . PHP_EOL ."----------------------" .PHP_EOL;
    file_put_contents('C:\wamp64\logs\lpa\lpalog_' .date("d.m.Y") . 'log',$log,FILE_APPEND);
}
function Show_user()
{
  global $displayUser;
